...
class ProjE:
    ...
    def __init__(self, data_dir, embed_dim=100, combination_method='simple', dropout=0.5, thr=0.5):
    ...
    def train(self, inputs, regularizer_weight=1., scope=None):
        with tf.variable_scope(scope or type(self).__name__) as scp:
            ...
            hrt_loss_p = -tf.reduce_sum(tf.log(tf.clip_by_value(hrt_res_softmax, 1e-10, 1.0)) * tf.maximum(0.,
                                                                                   hr_tlist_weight), 1)
            hrt_loss_n = -tf.reduce_sum(tf.log(tf.clip_by_value(1-hrt_res_softmax, 1e-10, 1.0)) * tf.maximum(0.,
                                                                                   -hr_tlist_weight), 1)
            hrt_loss_ = hrt_loss_p * self.thr / tf.reduce_sum(tf.maximum(0., hr_tlist_weight), 1) + hrt_loss_n * (1-self.thr) / tf.reduce_sum(tf.maximum(0., -hr_tlist_weight), 1)
            hrt_loss = tf.reduce_sum(hrt_loss_)

            self.trh_softmax = trh_res_softmax = self.sampled_softmax(trh_res, tr_hlist_weight)
            trh_loss_p = -tf.reduce_sum(tf.log(tf.clip_by_value(trh_res_softmax, 1e-10, 1.0)) * tf.maximum(0., tr_hlist_weight), 1)
            trh_loss_n = -tf.reduce_sum(tf.log(tf.clip_by_value(1-trh_res_softmax, 1e-10, 1.0)) * tf.maximum(0., -tr_hlist_weight), 1)

            trh_loss_ = trh_loss_p * self.thr / tf.reduce_sum(tf.maximum(0., tr_hlist_weight), 1) + trh_loss_n * (1 - self.thr) / tf.reduce_sum(tf.maximum(0., -tr_hlist_weight), 1)
            trh_loss = tf.reduce_sum(trh_loss_)
...
def main(_):
    ...
    parser.add_argument('--data', dest='data_dir', type=str, help="Data folder", default='./FB15K/')
    ...
    parser.add_argument("--model", dest='model', type=str, help="model_prefix", default='FB15K')
    ...
    parser.add_argument("--neg_weight", dest='neg_weight', type=float, help="Sampling weight on negative examples",
                        default=0.1)
    parser.add_argument("--gpu", dest='gpu', type=str, help="Sampling weight on negative examples",
                        default="2")
    parser.add_argument("--thr", dest='thr', type=float, help="Sampling weight on negative examples",
                        default=0.95)
    ...
    model = ProjE(args.data_dir, embed_dim=args.dim, combination_method=args.combination_method,
                  dropout=args.drop_out, thr=args.thr)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as session:
        ...
        for data_func, test_type in zip([model.validation_data, model.testing_data], ['VALID', 'TEST']):
            ...
            print(
                "[%s] INITIALIZATION [HEAD PREDICTION] MEAN RANK: %.1f FILTERED MEAN RANK %.1f MRR: %.3f FILTERED MRR %.3f HIT@10 %.3f FILTERED HIT@10 %.3f HIT@3 %.3f FILTERED HIT@3 %.3f HIT@1 %.3f FILTERED HIT@1 %.3f" %
                (test_type, np.mean(accu_mean_rank_h), np.mean(accu_filtered_mean_rank_h),
                 np.mean(list(map(lambda x: 1/x, accu_mean_rank_h))), np.mean(list(map(lambda x: 1/x, accu_filtered_mean_rank_h))),
            ...
            print(
                "[%s] INITIALIZATION [Tail PREDICTION] MEAN RANK: %.1f FILTERED MEAN RANK %.1f MRR: %.3f FILTERED MRR %.3f HIT@10 %.3f FILTERED HIT@10 %.3f HIT@3 %.3f FILTERED HIT@3 %.3f HIT@1 %.3f FILTERED HIT@1 %.3f" %
                (test_type, np.mean(accu_mean_rank_t), np.mean(accu_filtered_mean_rank_t),
                 np.mean(list(map(lambda x: 1/x, accu_mean_rank_t))), np.mean(list(map(lambda x: 1/x, accu_filtered_mean_rank_t))),
            ...
            if n_iter % args.eval_per == 0 or n_iter == args.max_iter - 1:
                ...
                for data_func, test_type in zip([model.validation_data, model.testing_data], ['VALID', 'TEST']):
                    ...
                    print(
                        "[%s] INITIALIZATION [HEAD PREDICTION] MEAN RANK: %.1f FILTERED MEAN RANK %.1f MRR: %.3f FILTERED MRR %.3f HIT@10 %.3f FILTERED HIT@10 %.3f HIT@3 %.3f FILTERED HIT@3 %.3f HIT@1 %.3f FILTERED HIT@1 %.3f" %
                        (test_type, np.mean(accu_mean_rank_h), np.mean(accu_filtered_mean_rank_h),
                         np.mean(list(map(lambda x: 1 / x, accu_mean_rank_h))),
                         np.mean(list(map(lambda x: 1 / x, accu_filtered_mean_rank_h))),
                         np.mean(np.asarray(accu_mean_rank_h, dtype=np.int32) <= 10),
                         np.mean(np.asarray(accu_filtered_mean_rank_h, dtype=np.int32) <= 10),
                         np.mean(np.asarray(accu_mean_rank_h, dtype=np.int32) <= 3),
                         np.mean(np.asarray(accu_filtered_mean_rank_h, dtype=np.int32) <= 3),
                    ...
                    print(
                        "[%s] INITIALIZATION [Tail PREDICTION] MEAN RANK: %.1f FILTERED MEAN RANK %.1f MRR: %.3f FILTERED MRR %.3f HIT@10 %.3f FILTERED HIT@10 %.3f HIT@3 %.3f FILTERED HIT@3 %.3f HIT@1 %.3f FILTERED HIT@1 %.3f" %
                        (test_type, np.mean(accu_mean_rank_t), np.mean(accu_filtered_mean_rank_t),
                         np.mean(list(map(lambda x: 1 / x, accu_mean_rank_t))),
                         np.mean(list(map(lambda x: 1 / x, accu_filtered_mean_rank_t))),
                         np.mean(np.asarray(accu_mean_rank_t, dtype=np.int32) <= 10),
                         np.mean(np.asarray(accu_filtered_mean_rank_t, dtype=np.int32) <= 10),
                         np.mean(np.asarray(accu_mean_rank_t, dtype=np.int32) <= 3),
                         np.mean(np.asarray(accu_filtered_mean_rank_t, dtype=np.int32) <= 3),
                    ...
                    if n_iter == args.max_iter - 1 and test_type=='TEST':
                        d = {}
                        d['mr_raw'] = (np.mean(accu_mean_rank_h)+np.mean(accu_mean_rank_t)) / 2
                        d['mr'] = (np.mean(accu_filtered_mean_rank_h)+np.mean(accu_filtered_mean_rank_t)) / 2
                        d['mrr_raw'] = (np.mean(list(map(lambda x: 1 / x, accu_mean_rank_h)))+ np.mean(list(map(lambda x: 1 / x, accu_mean_rank_t)))) / 2
                        d['mrr'] = (np.mean(list(map(lambda x: 1 / x, accu_filtered_mean_rank_h))) + np.mean(list(map(lambda x: 1 / x, accu_filtered_mean_rank_t)))) / 2
                        d['hit10_raw'] = (np.mean(np.asarray(accu_mean_rank_h, dtype=np.int32) <= 10) + np.mean(np.asarray(accu_mean_rank_t, dtype=np.int32) <= 10)) / 2
                        d['hit10'] = (np.mean(np.asarray(accu_filtered_mean_rank_h, dtype=np.int32) <= 10) + np.mean(np.asarray(accu_filtered_mean_rank_t, dtype=np.int32) <= 10)) /2
                        d['hit3_raw'] = (np.mean(np.asarray(accu_mean_rank_h, dtype=np.int32) <= 3) + np.mean(np.asarray(accu_mean_rank_t, dtype=np.int32) <= 3)) / 2
                        d['hit3'] = (np.mean(np.asarray(accu_filtered_mean_rank_h, dtype=np.int32) <= 3) + np.mean(np.asarray(accu_filtered_mean_rank_t, dtype=np.int32) <= 3)) / 2
                        d['hit1_raw'] = (np.mean(np.asarray(accu_mean_rank_h, dtype=np.int32) <= 1)+ np.mean(np.asarray(accu_mean_rank_t, dtype=np.int32) <= 1)) / 2
                        d['hit1'] = (np.mean(np.asarray(accu_filtered_mean_rank_h, dtype=np.int32) <= 1) + np.mean(np.asarray(accu_filtered_mean_rank_t, dtype=np.int32) <= 1)) / 2
                        with open('./log/'+args.model+'.json', 'w') as f:
                            json.dump(d, f)
