...
REAL hit1_raw, hit3_raw, hit10_raw, mr_raw, mrr_raw;
REAL hit1TC, hit3TC, hit10TC, mrTC, mrrTC;
REAL hit1TC_raw, hit3TC_raw, hit10TC_raw, mrTC_raw, mrrTC_raw;
extern "C"
void initTest() {
    lastHead = 0;
    lastTail = 0;
...
void test_link_prediction(bool type_constrain = false) {
    ...
    hit10_raw = (l_tot+r_tot) / 2;
    hit3_raw = (l3_tot+r3_tot) / 2;
    hit1_raw = (l1_tot+r1_tot) / 2;
    ...
    if (type_constrain) {
        ...
        mrrTC_raw = (l_reci_rank_constrain+r_reci_rank_constrain)/2;
        mrTC_raw = (l_rank_constrain+r_rank_constrain) / 2;
        hit10TC_raw = (l_tot_constrain+r_tot_constrain) / 2;
        hit3TC_raw = (l3_tot_constrain+r3_tot_constrain) / 2;
        hit1TC_raw = (l1_tot_constrain+r1_tot_constrain) / 2;
    }
}
...
extern "C"
REAL getTestLinkHit10raw(bool type_constrain = false) {
    if (type_constrain)
        return hit10TC_raw;
    printf("%f\n", hit10_raw);
    return hit10_raw;
...
extern "C"
REAL  getTestLinkHit3raw(bool type_constrain = false) {
    if (type_constrain)
        return hit3TC_raw;
    return hit3_raw;
}
...
extern "C"
REAL  getTestLinkHit1raw(bool type_constrain = false) {
    if (type_constrain)
        return hit1TC_raw;
    return hit1_raw;
}
...
extern "C"
REAL  getTestLinkMRraw(bool type_constrain = false) {
    if (type_constrain)
        return mrTC_raw;
    return mr_raw;
}
...
extern "C"
REAL  getTestLinkMRRraw(bool type_constrain = false) {
    if (type_constrain)
        return mrrTC_raw;
    return mrr_raw;
}
